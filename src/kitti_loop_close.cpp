...
int main(int argc, char** argv)
{
    ...
    perception_oru::NDTMap **map= new perception_oru::NDTMap * [12];
    for(unsigned int i=0;i<12;i++)
    {
        perception_oru::LazyGrid* grid = new perception_oru::LazyGrid(0.8);
        grid->semantic_index=i;
        map[i]=new perception_oru::NDTMap(grid,true);
        map[i]->guessSize(0,0,0,80,80,80);
        map[i]->computeNDTCells(CELL_UPDATE_MODE_SAMPLE_VARIANCE);
    }
    std::vector<perception_oru::NDTHistogram> histograms;
    std::ofstream out_file("loop.dat",ios::out|ios::binary);
    std::cout<<"Float size: "<<sizeof(float);
	for(int t=0; t<s_point; t+=10)
	{
        clock_t begin_time = clock();
		pcl::PointCloud<pcl::PointXYZI>::Ptr cloud = getB(pointcloud_files[t], label_files[t], label_map);
        std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr >laserCloud=matcher.getSegmentsFast(cloud);
        matcher.loadMap(map,laserCloud,80);
        perception_oru::NDTHistogram hist(map, 1, HIST_BINS1, HIST_BINS2,12,RANGE1, RANGE2);
        float time_diff=float( clock() -begin_time ) / CLOCKS_PER_SEC;
        std::cerr<<time_diff<<std::endl;
        float out_buff[histograms.size()];
        #pragma omp parallel num_threads(N_THREADS)
        {
            #pragma omp for
            for(int i=0;i<histograms.size();i++)
            {
                float out_val=(float) histograms.at(i).getSimilarity(hist);
                //int thread_id = omp_get_thread_num();
                out_buff[i]=out_val;
            }
        }
        if(t>0)
            out_file.write((char *) out_buff,sizeof(float)*histograms.size());
