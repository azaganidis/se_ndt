...
bool NDTMatcherP2D_SE::match( NDTMap **targetNDT,
...
//    double NORM_MAX = support_size/8, ROT_MAX = M_PI/18; //
	...
	pcl::PointCloud<pcl::PointXYZ> moving[NumInputs];
	for(int nIn=0;nIn<NumInputs;nIn++)
		if(subsample_size > 0) {
			moving[nIn] = subsample(source[nIn]);
			std::cout<<"s "<<source[nIn].points.size()<<std::endl;
			std::cout<<"d "<<moving[nIn].points.size()<<std::endl;
		} else {
...
pcl::PointCloud<pcl::PointXYZ> NDTMatcherP2D_SE::subsample(pcl::PointCloud<pcl::PointXYZ>& original)
{
...
bool NDTMatcherP2D_SE::update_score_gradient(Eigen::Matrix<double,6,1> &score_gradient,
...
void NDTMatcherP2D_SE::update_hessian(Eigen::Matrix<double,6,6> &Hessian,
        Eigen::Vector3d &transformed,
        Eigen::Matrix3d & Cinv,
...
void NDTMatcherP2D_SE::precomputeAngleDerivatives(Eigen::Vector3d &eulerAngles)
{
...
void NDTMatcherP2D_SE::computeDerivativesLocal(pcl::PointXYZ &pt,
        Eigen::Matrix<double,3,6> &_Jest,
        Eigen::Matrix<double,18,6> &_Hest,
        bool computeHessian)
{
...
double NDTMatcherP2D_SE::scorePointCloud(pcl::PointCloud<pcl::PointXYZ> *source,
        NDTMap **targetNDT)
{
    double score_here = 0;
    NUMBER_OF_ACTIVE_CELLS = 0;
...
void NDTMatcherP2D_SE::derivativesPointCloud(pcl::PointCloud<pcl::PointXYZ> *source,
        NDTMap **targetNDT,
...
double NDTMatcherP2D_SE::lineSearchMT(  Eigen::Matrix<double,6,1> &score_gradient_init,
...
int NDTMatcherP2D_SE::MoreThuente::cstep(double& stx, double& fx, double& dx,
...
double NDTMatcherP2D_SE::MoreThuente::min(double a, double b)
{
    return (a < b ? a : b);
}

double NDTMatcherP2D_SE::MoreThuente::max(double a, double b)
{
    return (a > b ? a : b);
}

double NDTMatcherP2D_SE::MoreThuente::absmax(double a, double b, double c)
{
    a = fabs(a);
    b = fabs(b);
    c = fabs(c);
...
double NDTMatcherP2D_SE::normalizeAngle(double a)
{
